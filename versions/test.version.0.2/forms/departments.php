<?php
   ...
   */
	...
	if($table!=""||$table!=null)
	{
		...
		$normalTable=mysql_real_escape_string($normalTable);
		...
		$createStudentTable="CREATE TABLE IF NOT EXISTS $table(
					...
					gender text NOT NULL) engine = innoDB";
		
		$createStudentTable=mysql_query($createStudentTable) or die(mysql_error()); 		//run the query
		
		$insertIndex="INSERT INTO tableindex VALUES('$normalTable')";		//insert department name into tableindex
		
		$insertIndex=mysql_query($insertIndex) or die(mysql_error());		//run the query
