$("#registerProfessor").submit(function(event){
	...
	if(isRegisterValid())
	{
			var $form = $(this),
				...
				if ($(".dbResponseSuccess").length <= 0){
					$form.prepend('<div class="dbResponseSuccess ajaxResponse successfulResponse"><span class="success"></span>Professor Added Successfully</div>');
					$form.prepend('<div class="dbResponseFailure ajaxResponse failedResponse"><span class="failure"></span>Professor Addition Failed</div>');
			...
			$.ajax({
				url: "forms/registerProfessor.php",
				type: "post",
				beforeSend:function(){
					$('.ajaxResponse:not(#registerProfessor .ajaxResponse)').remove();
				},
				data: serializedData,
