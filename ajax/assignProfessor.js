$("#assignProfessor").submit(function(event){
	
	isAssignProfessorValid();

	if(isAssignProfessorValid())
	{
			var $form = $(this),
			...
			if ($(".dbResponseSuccess").length <= 0){
				$form.prepend('<div class="dbResponseSuccess ajaxResponse successfulResponse"><span class="success"></span>Student Added Successfully</div>');
				$form.prepend('<div class="dbResponseFailure ajaxResponse failedResponse"><span class="failure"></span>Student Addition Failed</div>');
			...
			$.ajax({
				url: "forms/assignProfessor.php",
				type: "post",
				beforeSend:function(){
					$('.ajaxResponse:not(#assignProfessor .ajaxResponse)').remove();
				},
				data: serializedData,
